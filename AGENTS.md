...
**路径**：`~/.micro-agent/history/YYYY-MM-DD.md`（Markdown 格式）

**用途**：人类可读的对话历史摘要，支持搜索和回顾

**特点**：
- 不加载到上下文（与 sessions/*.jsonl 不同）
- 按日期分文件，便于管理和搜索
- 通过 `history` 工具搜索和读取

#### 历史记录整理机制

**触发条件**：上下文 token 数超过 `contextWindowTokens × threshold`
...
sessions:
  ...
  history:
    enabled: true           # 是否启用历史整理
    threshold: 0.7          # 触发阈值（相对于 contextWindow）
    keepRecentMessages: 10  # 保留最近消息数
    targetRatio: 0.5        # 整理后上下文目标比例
```

**整理流程**：
1. 检测上下文是否超过阈值
2. 选择整理边界（在 user 消息处截断）
3. 调用 LLM 生成历史条目
4. 写入 `history/YYYY-MM-DD.md`
5. 整理后的消息不再加载到上下文
...
```json
{"action": "search", "query": "JWT"}
{"action": "search", "query": "认证|授权", "useRegex": true}
{"action": "list", "dateFrom": "2026-03-01", "dateTo": "2026-03-16"}
{"action": "read", "date": "2026-03-16"}
```

### 用户自定义技能

**路径**：`~/.micro-agent/skills/<skill-name>/SKILL.md`
